...
const UserProfile = () => {  
  const id = "6479b3106146b1e154a2c94a";
  useEffect(() => {
    axios.get("http://localhost:4000/get_user/"+id)
    .then((res) => {
      const { FirstName, SecondName, Age, Dob, Address, PhoneNumber, Institution, Course, EmailId, Password } = res.data;
      setInput({
        FirstName: FirstName,
  ...
  const handleSubmit = (e) => {
    ...
    axios.put("http://localhost:4000/update", input)
      .then((res) => {
